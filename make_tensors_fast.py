...
def make_tensors_fast(
    ...
    """
    N = signal.ndims
    if N==2 & signa.shape[-1] == 1:
        N=1
    if spatial_size<1:
        except ValueError('What use would such a small kernel be?')
    ...
    if region_of_interest is None:
        if N ==1:
            region_of_interest = np.array([[0], [signal.shape[0]]])
        else:
            region_of_interest = np.array([[0,]*N, list(signal.shape)])
    ...
    if N==1:
        ...
        conv_results = np.zeros(list(np.diff(region_of_interest, axis=0)[:,0])+[3,])
        for i, kern in enumerate([kernelx0, kernelx1, kernelx2]):
            conv_results[:,i] = conv3(signal, kern, roix) #find what conv3 is doing
        ...
        # T=[3*3+2*2].
        T = np.zeros(list(np.diff(region_of_interest, axis=0)[:,0])+[1, 1])
        T[:,1,1] = conv_results[:,2]**2 + gamma * conv_results[:,1]**2
        T = np.sqeeze(T)
        ...
        roiy[0] = max(roiy[0], 0)
        roiy[1] = min(roiy[1], len(signal))
        convy_results = np.zeros(list(np.diff(region_of_interest, axis=0)[:,0])+[3])
        for i, kern in enumerate([kernely0, kernely1, kernely2]):
            convy_results[:,i] = conv3(signal, kern, roiy) #find what conv3 is doing

        #Convolutions in the x-direction.
        kernelx0 = np.copy(kernely0)
        kernelx1 = np.copy(kernely1)
        kernelx2 = np.copy(kernely2)
        roix = region_of_interest;
        roix = roix[:convy_results.ndims,:]
        roix[1:] = roix[1:] + 1 - np.repeat(roix[:,0], [1 2])
        convy_results = np.zeros(list(np.diff(region_of_interest, axis=0)[:,0])+[6])

        conv_results[:,:,0] = conv3(conv_y0, kernelx0, roix)
        conv_results[:,:,1] = conv3(conv_y0, kernelx1, roix)
        conv_results[:,:,3] = conv3(conv_y0, kernelx2, roix)
        clear('conv_y0')
        conv_results[:,:,2] = conv3(conv_y1, kernelx0, roix)
        conv_results[:,:,5] = conv3(conv_y1, kernelx1, roix) / 2
        clear('conv_y1')
        conv_results[:,:,4] = conv3(conv_y2, kernelx0, roix)
        clear('conv_y2')
        # Apply the inverse metric.
        conv_results[:,:,1] = Qinv[1,1] * conv_results[:,:,1]
        ...
        T = np.zeros((np.array([diff(np.transpose(region_of_interest)) + 1,2,2]),np.array([diff(np.transpose(region_of_interest)) + 1,2,2])))
        T[:,:,1,1-1] = conv_results(:,:,4) ** 2 + conv_results(:,:,6) ** 2 + gamma * conv_results(:,:,2) ** 2
        T[:,:,2,2-1] = conv_results(:,:,6) ** 2 + conv_results(:,:,5) ** 2 + gamma * conv_results(:,:,3) ** 2
        T[:,:,1,2-1] = np.multiply((conv_results(:,:,4) + conv_results(:,:,5)),conv_results(:,:,6)) + np.multiply(gamma * conv_results(:,:,2),conv_results(:,:,3))
        T[:,:,2,1-1] = T(:,:,1,2)
        T = squeeze(T)
    elif N == 3:
        # Set up applicability and basis functions.
