...
"""
    ...
    for index in np.ndindex(shape):
        ...
        params[index] = lstsq(A[index], b[index], rcond=-1)[0]
    return params
...
displacement values.
    ...
    if model == 'constant':
        # Q is composed of (N,N) submatrices, corresponding to an element of AA
        # (symmetric). Thus there are a lot of repeated coefficients.
        ...
        for k,l in zip(*np.triu_indices(N)):
            Q[...,k, l] = normalized_conv(AA[...,k,l], cin, app, cinaver)
            # exploit symmetry of AA
            if k != l:
                Q[...,l, k] = Q[...,k, l]

        # q is composed of vectors of size (N,), each corresponding to an element of Ab.
        # Here there is no symmetry involved, but we follow the same procedure as in Q for clarity
        q = np.zeros(shape+(N,))
        for k in range(N):
            q[...,k] = normalized_conv(Ab[...,k], cin, app, cinaver)

        # Solve the equation Qp=q. (Eq. 7.23) where p is the unknown displacement
        displacement = lstsq_ND(Q, q)
        ...
        cout = coeff - np.sum(displacement*q, -1)
